    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        *********************************************************
        ...
        def get_user2gender(dataset):
            file = open("../Data/" + dataset + "/user2gender_map.txt", 'r')
        ...
        uid_gender, gender2name = get_user2gender(args.dataset)
        #for i, users_to_test in enumerate(users_to_test_list[-1]):
        #users_to_test_list = users_to_test[-1]
        male_users = [user for user in users_to_test if uid_gender[user] == 0]
        female_users = [user for user in users_to_test if uid_gender[user] == 1]
        ret = {"Male": {}, "Female": {}, "All": {}}
        ret["All"] = test(sess, model, users_to_test, drop_flag=False, batch_test_flag=batch_test_flag)
        ret["Male"] = test(sess, model, male_users, drop_flag=False, batch_test_flag=batch_test_flag)
        ret["Female"] = test(sess, model, female_users, drop_flag=False, batch_test_flag=batch_test_flag)

        with open("output/" + args.dataset + "/gender_metrics_" + args.model_type + "epoch=" + str(epoch) + ".csv", mode="w+") as file:
            writer = csv.writer(file)
            #metric, group, data
            header = ["metric", "group", "data"]
            writer.writerow(header)
            for group_id, group_name in gender2name.items():
                recall = sum(ret[group_name]["recall"])/len(ret[group_name]["recall"])
                ndcg = sum(ret[group_name]["ndcg"])/len(ret[group_name]["ndcg"])
                hit = sum(ret[group_name]["hit_ratio"])/len(ret[group_name]["hit_ratio"])
                precision = sum(ret[group_name]["precision"])/len(ret[group_name]["precision"])
                writer.writerow(["recall", group_name, recall])
                writer.writerow(["ndcg", group_name, ndcg])
                writer.writerow(["hit", group_name, hit])
                writer.writerow(["precision", group_name, precision])
        file.close()
